...
def Freq2LogFreqMapping(tfr, f, fr, fc, tc, NumPerOct):
    StartFreq = fc
    StopFreq = 1/tc
    Nest = int(np.ceil(np.log2(StopFreq/StartFreq))*NumPerOct) # int(np.ceil(np.log2(1/tc/fc)*NumPerOct))
    central_freq = []
    ...
    # 174 filters for 174 central frequencies
...
def cfp_feature_extraction(audio: np.array) -> np.array:
    ...
    h3 = scipy.signal.windows.blackmanharris(743) # window size - 2048
    h2 = scipy.signal.windows.blackmanharris(372) # window size - 1024
    h1 = scipy.signal.windows.blackmanharris(186) # window size - 512
...
def full_feature_extraction(
    ...
    Z1 = tfrLF1 * tfrLQ1
    ZN1 = (Z1 - np.mean(Z1)) / np.std(Z1) # normalize
    Z2 = tfrLF2 * tfrLQ2
    ZN2 = (Z2 - np.mean(Z2)) / np.std(Z2) # normalize
    Z3 = tfrLF3 * tfrLQ3
    ZN3 = (Z3 - np.mean(Z3)) / np.std(Z3) # normalize
...
def melody_feature_extraction(x):
    ...
    x = x.astype('float32')
    Hop = 320 # hop size (in sample)
    h = scipy.signal.windows.blackmanharris(2048) # window size - 2048
    fr = 2.0 # frequency resolution
    fc = 80.0 # the frequency of the lowest pitch
    tc = 1/1000.0 # the period of the highest pitch
    g = np.array([0.24, 0.6, 1])
