...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--use_amp",
        action="store_true",
        help="Use PyTorch native AMP (Automatic Mixed Precision) for faster inference. Uses ~50%% less GPU memory and ~20-30%% faster. Only works on CUDA devices."
        )
    parser.add_argument(
        "-bz",
        "--batch_size",
    ...
    parser.add_argument(
        ...
        type=str,
        default="O0",
        help="[DEPRECATED] Amp level for inferencing. No longer used (NVIDIA Apex not required)."
        )
    parser.add_argument(
        "-mt",
        "--model_type",
        type=str,
        default="pyramidnet",
        choices=['pyramidnet', 'gru_attention', 'simplified_gru_attention'],
        help="Model architecture to use: 'pyramidnet' (default, original), 'gru_attention' (GRU+Attention), or 'simplified_gru_attention' (simpler version)."
        )
